...
export const TestimonialsOverview = () => {
  return (
    <section className="w-full py-16 px-4 md:px-14 bg-[var(--color-surface-main)] flex flex-col items-center gap-12 overflow-hidden">
      {/* Header Section */}
      <div className="max-w-[1088px] flex flex-col items-center gap-2 text-center">
      ...
      {/* Testimonials Grid */}
      <div className="relative w-full max-w-[1200px] grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10">
        {/* 
            Mobile: Max 4 cards
            Desktop: Max 6 cards 
        */}
        {TESTIMONIALS_DATA.slice(0, 6).map((testimonial, index) => (
          <div 
            key={testimonial.id} 
            className={cn(
              "w-full max-w-[320px] mx-auto",
              // Hide cards 5 and 6 on mobile (screen < 1024px)
        ...
        {/* Gradient Fade Overlay for Desktop (as per design) */}
      ...
      {/* CTA Button */}
      <Button 
        className="bg-[var(--color-button-primary-brand-default)] text-[var(--color-surface-main)] hover:bg-[var(--color-button-primary-brand-hover)] rounded-sm px-6 py-2.5 text-sm font-semibold transition-colors border border-[var(--color-border-brand-default)]"
      >
